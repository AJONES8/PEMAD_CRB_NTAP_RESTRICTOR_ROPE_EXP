...
station_table_2 <- Station %>% 
                        ...
                        mutate(PAIR=str_sub(STATION,-4)) %>%
                        mutate(Restrictor=str_sub(PAIR,1,1)) %>%
                        mutate(Restrictor = case_when(Restrictor == '9' ~ 'Yes',TRUE ~ 'No')) %>% #skimr::skim()
                        mutate(theta=((DoorWidth * 3.28084)/2)/210,
                            bridle_angle = asin(theta),bridle_angle = bridle_angle * ( 180.0 / pi )) %>%
                        group_by(Restrictor,SEASON) %>% 
                        summarise(Stations = n_distinct(STATION),
                                  DEPTH_mean = round(mean(DepthBeg*0.3048),0),
                                  VSPEED_mean = round(mean(VSPEED * 0.514444),2),
                                  TowDistTrack_mean= round(mean(TowDistTrack),0),
                                  NetHeight_mean = round(mean(NetHeight),1),
                                  NetWidth_mean = round(mean(NetWidth),1),
                                  CableOut_S_mean = round(mean(CableOut_S * 1.8288),0),
                                  DoorWidth_mean = round(mean(DoorWidth),1),
                                  bridle_angle_mean = round(mean(bridle_angle),1),
                                  DEPTH_sd = sd(DepthBeg),
                                  VSPEED_sd = sd(VSPEED),
                                  TowDistTrack_sd = sd(TowDistTrack),
                                  NetHeight_sd = sd(NetHeight),
                                  NetWidth_sd = sd(NetWidth),
                                  CableOut_S_sd = sd(CableOut_S),
                                  DoorWidth_sd = sd(DoorWidth),
                                  bridle_angle_sd = sd(bridle_angle),
                                  DEPTH_se = round(DEPTH_sd/sqrt(Stations),1),
                                  VSPEED_se = round(VSPEED_sd/sqrt(Stations),2),
                                  TowDistTrack_se = round(TowDistTrack_sd/sqrt(Stations),0),
                                  NetHeight_se = round(NetHeight_sd/sqrt(Stations),2),
                                  NetWidth_se = round(NetWidth_sd/sqrt(Stations),2),
                                  CableOut_S_se = round(CableOut_S_sd/sqrt(Stations),1),
                                  DoorWidth_se = round(DoorWidth_sd/sqrt(Stations),1),
                                  bridle_angle_se = round(bridle_angle_sd/sqrt(Stations),2)
                                  )

station_table_2 <- station_table_2 %>% dplyr::select(SEASON,Stations,
                                                     DEPTH_mean,DEPTH_se,
                                                     VSPEED_mean,VSPEED_se,
                                                     TowDistTrack_mean,TowDistTrack_se,
                                                     NetHeight_mean,NetHeight_se,
                                                     NetWidth_mean,NetWidth_se,
                                                     CableOut_S_mean,CableOut_S_se,
                                                     DoorWidth_mean,DoorWidth_se,
                                                     bridle_angle_mean,bridle_angle_se)
...
length_mod_data <- Individual %>% 
                      filter(VIMSCODE %in% c(1028,4,1,171,44,6,15)) %>%
                      #filter(VIMSCODE %in% c(6)) %>%
                      #::skim()
                      left_join(.,Station) %>% 
                      #nrow()
                      uncount(round(ExpFactMeas,0)) %>%
                      ...
                      left_join(.,station_pair_current %>% dplyr::select(PAIR,CURRENT_DIF)) %>%
                      left_join(.,pair_zen_data %>% dplyr::select(PAIR,zenith)) %>%
                      group_by(PAIR,VIMSCODE,SEASON,COMMON,CURRENT_DIF,zenith,LENGTH) %>% 
                      summarise(N=sum(N),Y=sum(Y),TOT = sum(N,Y),PROP=Y/TOT) %>%
                      group_by(COMMON) %>%
                      mutate(LENGTH_NTILE = ntile(LENGTH,100)) %>%
                      filter(LENGTH_NTILE > 5,LENGTH_NTILE < 90) %>%
                      mutate(LENGTH_CM = round(LENGTH/10,0)) %>%
                      ungroup() 
...
# Using the SELECT package for length-based analyses
##Installation of the package and setup
...
palette <- ggthemes_data[["tableau"]][["color-palettes"]][["regular"]][[1]]


#Making data for rug plots
Rug_dat <- Individual %>% 
                filter(VIMSCODE %in% c(1028,4,1,171,44,6,15)) %>%
                left_join(.,Station) %>% 
                uncount(round(ExpFactMeas,0)) %>% 
                left_join(.,NM_Species_Code_List_Color_Coordinated %>%
                                  mutate(VIMSCODE=as.numeric(SPCODE))) %>%
                      dplyr::select(LENGTH,Restrictor,COMMON) %>%
                group_by(COMMON) %>%
                mutate(LENGTH_NTILE = ntile(LENGTH,100)) %>%
                      filter(LENGTH_NTILE > 5,LENGTH_NTILE < 95) %>%
                ungroup()
...
## Running the analyses and making output

```{r}
#Making objects for the analysis
SELECTplots <- NULL
SELECTplots <- list()
Permplots <- NULL
Permplots <- list()
...
#Making a list of each species
spp <- length_mod_data$COMMON %>% unique()
#season <- length_mod_data$SEASON %>% unique() 
season <- length_mod_data %>% mutate(SEASON='All') %>% .$SEASON %>% unique()

#Looping through each season and species
#For the manuscript we just report species
#Qualitative results are similar

for(k in 1:length(season)) {
for(i in 1:length(spp)) {

#Making a data set for each species
#Giving the data  column names needed for SELECT
SimCatch_spp <-  length_mod_data %>% filter(COMMON == spp[i]) %>% 
  mutate(SEASON='All') %>%
  filter(SEASON==season[k]) %>%
  mutate(N=log(N+1),Y=log(Y+1)) %>% #Log transforming the catch data to reduce the impact of outliers
  #mutate(N=sqrt(N),Y=sqrt(Y)) %>%
  #mutate(LENGTH = round(LENGTH/20,0)) %>% 
  #ggplot(aes(x=LENGTH,y=LENGTH_CM)) + geom_point()
  dplyr::select(lgth = LENGTH,n2=Y,n1=N,TowID=PAIR) #%>% #View()

SimCatch <- SimCatch_spp

SimCatch %>% skimr::skim()

#Setting up the spline fitting in SELECT
#To reduce overfitting a spline penalty of 4 is used this is high, but we think warranted given the spread of the data
Vnames=c("lgth","n1","n2")
#SplineFit=SplineSELECT(SimCatch,var.names=Vnames)
SplineFit=SplineSELECT(SimCatch,var.names=Vnames,bs='ps',quasi=TRUE,rm.zeros=TRUE)
summary(SplineFit)$r.sq

#Making a table of results
sm <- smooth_estimates(SplineFit) |>
  add_confint() %>%
  mutate(estimate = inv.logit(.estimate)) %>%
  mutate(lower_ci = inv.logit(.lower_ci)) %>%
  mutate(upper_ci = inv.logit(.upper_ci))

SELECToutput <- SELECToutput %>% rbind(sm %>% mutate(COMMON = spp[i],SEASON = season[k]))

#Permuting the data for the test
permRsq=function(data,var.names) {
SplineFit=SplineSELECT(data,var.names)
summary(SplineFit)$r.sq
}

ObsRsq[i]=permRsq(SimCatch,Vnames)
cat("\nThe observed r-square is",ObsRsq,"\n")

PermRsq[[i]]=permSELECT(SimCatch,Vnames,permRsq,haul="TowID",paired=T,nsim=100)

#Testing the to generate a pvalue
pval=mean(ObsRsq[i]<=PermRsq[[i]])

pvaloutput <- pvaloutput %>% rbind(.,pval %>% as.data.frame() %>% mutate(COMMON = spp[i],SEASON = season[k]))

cat("\nThe p-value for length-independent catch share is",pval,"\n") %>% print()

SplineFit$gcv.ubre

#Making output to plot
Permoutput <- Permoutput %>% rbind(PermRsq[[i]] %>% as.data.frame() %>% mutate(COMMON = spp[i],SEASON = season[k]))
ObsRsqoutput <- ObsRsqoutput %>% rbind(ObsRsq[i] %>% as.data.frame() %>% mutate(COMMON = spp[i],SEASON = season[k]))

}}


#Plotting things out
#First the spline that was fit for each species
library(patchwork)
library(ggdensity)
a <- SELECToutput |> 
  filter(.smooth == "s(lgth)") |>
  ggplot() +
  geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci, x = lgth, fill=COMMON,group=SEASON),
    alpha = 0.2
  ) +
  geom_line(aes(x = lgth, y = estimate,group=SEASON,colour=COMMON,linetype = SEASON), lwd = 1.2) +
  #facet_grid(SEASON~COMMON,scales='free') +
  facet_grid(.~COMMON,scales='free') +
  scale_color_tableau() +
  scale_fill_tableau() +
  geom_hline(yintercept = 0.5) +
  geom_hdr_rug(data = Rug_dat %>% filter(Restrictor == 'N') %>%
                 #mutate(LENGTH = round(LENGTH/20,0)) %>%
                 mutate(lgth = LENGTH) %>%
                 mutate(SEASON='All'), 
               mapping = aes(x = lgth), sides = "b", length = unit(.8, "cm"),
               inherit.aes = FALSE,method = "histogram",
               probs = c(0.99, 0.95, 0.8, 0.5)) + 
  geom_hdr_rug(data = Rug_dat %>% filter(Restrictor != 'N') %>%
                 #mutate(LENGTH = round(LENGTH/20,0)) %>%
                 mutate(lgth = LENGTH) %>%
                 mutate(SEASON='All'), 
               mapping = aes(x = lgth), sides = "t", length = unit(.8, "cm"),
               inherit.aes = FALSE,method = "histogram",
               probs = c(0.99, 0.95, 0.8, 0.5)) + 
  theme(plot.margin = margin(0, 0, 0, 0, "pt"),legend.position = 'none') +
  labs(x='Length (cm)',y='Catch Proportion')

#The permuted distribution of R^2 values
#Compared to the observed value
mylabel = bquote(italic(R)^2)
b <- Permoutput %>% 
  ggplot(aes(x=V1)) + 
  geom_histogram() + 
  geom_vline(data=ObsRsqoutput,aes(xintercept = .,linetype = SEASON,colour=COMMON),lwd = 1.2) +
  facet_grid(.~COMMON,scales='free') +
  #facet_grid(SEASON~COMMON,scales='free') +
    labs(title = '') +
  labs(y = "Density",x=mylabel) +
  scale_color_tableau() +
  theme(legend.position='none',
        strip.background = element_blank(),
        strip.text.x = element_blank()) +
  theme(margin(0, 0, 0, 0, "pt"))

select_out <- a / b & plot_annotation(tag_levels = 'A')

select_out

pvaloutput %>% arrange(COMMON) %>% dplyr::select(-SEASON)
...
#LENGTH_NTILE model
length_mod_bi_2a <- gam(#Y/TOT ~ s(LENGTH_NTILE) + 
                         #PROP ~ s(LENGTH_NTILE,k=5) + 
                        Y/(N+Y) ~ s(LENGTH_NTILE) + 
                        COMMON +
                        s(LENGTH_NTILE,COMMON,bs='sz',k=5) +
                        s(PAIR,bs='re'),
                        #s(LENGTH_NTILE,PAIR,bs='sz'),
                      weights = N+Y,
                      data=length_mod_data %>% #filter(COMMON == "longfin inshore squid") %>%
                        #filter(!PAIR %in% c('F479','F547')) %>% without pairs that were large outliers in catch weights
...
preds_gam <- length_mod_bi_2a %>% ggeffects::predict_response(terms = c('LENGTH_NTILE','COMMON'),
         #length_mod_bi_2a %>% ggeffects::predict_response(terms = c('LENGTH_NTILE'),
                                                          #exclude = 's(PAIR)',
                                                          #margin = "mean_mode",
                                                          #margin = "empirical",
                                                          ...
                                                          ) %>% plot(show_residuals = TRUE) + facet_wrap(~group) + geom_hline(yintercept = 0.50)

preds_gam %>% as.data.frame() %>%
  ggplot(aes(x=x,y=predicted,color=group,group=facet)) + geom_line() + 
  facet_wrap(~group)
...
terms=c('LENGTH_NTILE','COMMON'), margin = 'empirical',type="count")
...
e <- gear_data %>% 
  #mutate(Restrictor = case_when(Restrictor == 'Y' ~ 'Yes',TRUE ~ 'No')) %>%
  mutate(Restrictor = case_when(Restrictor == 'Y' ~ 'Restricted',TRUE ~ 'Standard')) %>%
  ggplot(aes(y=SCOPE,x=DepthEnd,color=Restrictor)) + geom_point() +
  scale_color_tableau(direction = -1) +
  #scale_x_discrete(limits = rev) +
  #theme(legend.position = 'None') +
  labs(y='Scope',x='')
...
# Gear LMM for revision

```{r}
#Updating this
gear_data_2 <- Station %>% mutate(PAIR=str_sub(STATION,-3)) %>%
                            mutate(SEASON=case_when(month(DATE) < 7 ~ 'S',TRUE~'F')) %>%
                            mutate(PAIR=paste(SEASON,PAIR,sep='')) %>%
                        dplyr::select(STATION,DATE,PAIR,SEASON,Restrictor,DoorWidth,SCOPE,CableOut_P,CableOut_S,
                                LATITUDE_END,NetWidth,NetHeight,TOWDIST,VSPEED,DepthEnd,DepthBeg) %>%
                        mutate(SEASON=case_when(SEASON == 'S' ~ 'SPRING',TRUE~'FALL')) %>%
                        mutate(VSPEED_MS = VSPEED * 0.514)


gear_data_2$SEASON <- factor(gear_data$SEASON, levels = c("SPRING", "FALL"))

library(GGally)

#Visualizing the variables going into the models
e2 <- gear_data_2 %>% 
  mutate(Restrictor = case_when(Restrictor == 'N' ~ 'Standard',TRUE ~ 'Restricted')) %>%
  filter(NetWidth>12) %>%
  filter(NetHeight < 6.25) %>% 
  filter(VSPEED > 2.5) %>% 
  filter(DoorWidth > 30) %>%
  mutate(theta=((DoorWidth * 3.28084)/2)/210,
         bridle_angle = asin(theta),bridle_angle = bridle_angle * ( 180.0 / pi )) %>%
  rename('Depth' = DepthEnd,'Speed' = VSPEED,'Scope' = SCOPE,'Cable out' =  CableOut_P) %>%
  #dplyr::select(Depth,Speed,Scope,`Cable out`,bridle_angle,DoorWidth,NetWidth,NetHeight,Restrictor) %>%
  dplyr::select(Depth,Speed,NetHeight,NetWidth,DoorWidth,Restrictor) %>%
  ggpairs(ggplot2::aes(colour = Restrictor)) +
  scale_color_tableau(direction = -1) +
  scale_fill_tableau(direction = -1)

e2 + labs(title='E') + theme(axis.title = element_text(face="bold"))

#Fitting models for each performance metric and plotting them out
library(lme4)
library(sjPlot)
a2 <- gear_data_2 %>% 
  mutate(Restrictor = case_when(Restrictor == 'Y' ~ 'Restricted',TRUE ~ 'Standard')) %>%
  ...
  filter(NetWidth>12) %>%
  filter(VSPEED > 2.5) %>% 
  rename('Depth' = DepthEnd,'Speed' = VSPEED_MS,'Scope' = SCOPE,'Cable out' =  CableOut_P) %>%
  mutate(Restrictor = as.factor(Restrictor)) %>%
  mutate(Restrictor = fct_relevel(Restrictor,rev)) %>% 
  lmer(NetHeight ~ 
                  Restrictor +
                  `Depth` + 
                   `Speed` + 
                   (1 | DATE), data = .) %>%
  #tab_model(.)
  plot_model(.,show.values = TRUE,sort.est = TRUE,title = 'Net height (m)')
...
b2 <- gear_data_2 %>% 
  ...
  lmer(NetWidth ~ 
                  ...
                  `Depth` + 
                   ...
                   (1 | DATE), data = .) %>%
  #tab_model(.)
  plot_model(.,show.values = TRUE,sort.est = TRUE,title = 'Net width (m)')
...
c2 <- gear_data_2 %>% 
  ...
  lmer(DoorWidth ~ 
                  ...
                  `Depth` + 
                   ...
                   (1 | DATE), data = .) %>%
  #tab_model(.)
  plot_model(.,show.values = TRUE,sort.est = TRUE,title = 'Door spread (m)')
...
d2 <- gear_data_2 %>% 
  ...
  lmer(bridle_angle ~ 
                   ...
                   (1 | DATE), data = .) %>%
  #tab_model(.)
  plot_model(.,show.values = TRUE,sort.est = TRUE) +
  labs(title = expression('Bridle angle'~({o})))
...
# 
col_for_list_2 <- list(Restrictor = md("**Restrictor (Y/N)**"),
                     Stations = md("**Stations**"),
                     SEASON = md("**Season**"),
                     DEPTH_mean = md("**Depth (m)**"),
                     DEPTH_se = md(" "),
                     VSPEED_mean = md("**Vessel speed (m/s)**"),
                     VSPEED_se = md(" "),
                     TowDistTrack_mean = md("**Tow distance (m)**"),
                     TowDistTrack_se = md(" "),
                     NetHeight_mean = md("**Net height (m)**"),
                     NetHeight_se = md(" "),
                     NetWidth_mean = md("**Net width (m)**"),
                     NetWidth_se = md(" "),
                     DoorWidth_mean = md("**Door width (m)**"),
                     DoorWidth_se = md(" "),
                     NetWidth_mean = md("**Net width (m)**"),
                     NetWidth_se = md(" "),
                     CableOut_S_mean = md("**Cable out (m)**"),
                     CableOut_S_se = md(" "),
                     bridle_angle_mean = md("**Bridle angle (o)**"),
                     bridle_angle_se = md(" ")
                     )
station_table_2 %>%
  mutate(DEPTH_mean=DEPTH_mean) %>%
          ...
          locations = cells_body(columns = c('DEPTH_se','VSPEED_se',
                                             'TowDistTrack_se','NetHeight_se',
                                             'DoorWidth_se','NetWidth_se','CableOut_S_se',
                                             'bridle_angle_se',
                                             ))) %>%
          cols_label(.list = col_for_list_2)
